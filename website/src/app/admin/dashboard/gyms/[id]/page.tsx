...
export default function AdminGymDetailPage() {
  ...
  const fetchGym = async () => {
    try {
      ...
      const res = await fetch(`http://localhost:8000/api/admin/gyms/${gymId}`, {
  ...
  const handleSaveEconomy = async () => {
    ...
    try {
      const token = localStorage.getItem("zonofit_portal_token");
      await fetch(`http://localhost:8000/api/admin/gyms/${gymId}/economy`, {
        method: "PUT",
        headers: { 
  ...
  const handleCreatePlan = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("zonofit_portal_token");
      await fetch(`http://localhost:8000/api/admin/gyms/${gymId}/plans`, {
        method: "POST",
        headers: { 
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json"
  ...
  const handleDeletePlan = async (planId: string) => {
    ...
    try {
      const token = localStorage.getItem("zonofit_portal_token");
      await fetch(`http://localhost:8000/api/admin/gyms/${gymId}/plans/${planId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchGym();
